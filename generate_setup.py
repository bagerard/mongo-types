...
try:
    from poetry.masonry.builders.editable import SdistBuilder
...
factory = Factory()
poetry = factory.create_poetry(os.path.dirname(__file__))  # type: ignore

# Use the SdistBuilder to genrate a blob for setup.py
sdist_builder = SdistBuilder(poetry, None, None)  # type: ignore
setuppy_blob = sdist_builder.build_setup()

with open("setup.py", "wb") as unit:
    unit.write(
        b"# -------- DO NOT EDIT: This setup.py was autogenerated using poetry. -------- \n"
    )
    unit.write(
        b"# -- This is used for back compatibility with Python version before PEP 660 -- \n\n"
    )
